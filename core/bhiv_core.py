...
# MongoDB ratings collection
from .mongodb import get_collection
from .mongo_models import rating_document
...
def notify_on_rate(content_id: str, rating_payload: Dict[str, Any], regen_threshold: int = 2) -> Dict[str, Any]:
    ...
    # persist to MongoDB and compute aggregates
    ...
    try:
        ...
        # Compute aggregates via MongoDB aggregation
        pipeline = [
            {"$match": {"content_id": content_id}},
            {"$group": {"_id": None, "avg_rating": {"$avg": "$rating"}, "cnt": {"$sum": 1}}},
        ]
        ...
        logger.exception("Failed to persist rating to MongoDB for content=%s", content_id)
        avg_rating = None
        total = 0
